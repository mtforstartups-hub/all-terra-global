                ...
                <div className="w-full lg:w-1/2 bg-[#0a0a0a] flex items-center justify-center py-20 lg:py-0 px-6 lg:px-12">
                    <div className="relative w-full max-w-md h-[250px]">
                        {edges.map((edge, index) => (
                            <div
                                key={edge.title}
                                className={`absolute inset-0 bg-white/5 backdrop-blur-sm p-6 rounded-2xl border border-white/10 transition-all duration-500 ${activeIndex === index
                        ...
                        <div className="absolute -bottom-16 left-0 right-0 flex justify-center gap-4">
                            <button
                                onClick={() => setActiveIndex((prev) => (prev - 1 + edges.length) % edges.length)}
                                className="w-10 h-10 rounded-full border border-white/20 bg-white/5 flex items-center justify-center text-white/60 hover:border-[#F8AB1D] hover:text-[#F8AB1D] transition-colors"
                            >
                            ...
                            <button
                                onClick={() => setActiveIndex((prev) => (prev + 1) % edges.length)}
                                className="w-10 h-10 rounded-full border border-white/20 bg-white/5 flex items-center justify-center text-white/60 hover:border-[#F8AB1D] hover:text-[#F8AB1D] transition-colors"
                            >
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
